...
def build_only_N():
    ...
    xticks = np.arange(100, 30100, 100)
    ...
    plt.plot(x[:-2], naive_B_calls, 'bo', label = 'Naive Build')
    plt.plot(x, ucb_B_calls, 'ro', label = 'UCB Build')
    plt.plot(xticks, quadratic_naive, 'b-', label = "quadratic fit")
    plt.plot(xticks, nlogn, 'r-', label = "nlogn fit")
...
def build_only_k():
    ...
    plt.plot(k[:-2], naive_B_calls, 'bo', label = "Naive")
    plt.plot(k, ucb_B_calls, 'ro', label = "UCB")
    plt.plot(k, linear_naive, label = "Linear naive fit")
    plt.plot(k, linear_ucb, label = "Linear UCB fit")
    plt.legend()
    plt.title("Number of distance calls in BUILD step vs. k")
    plt.xlabel("k")
...
def build_and_swap_N():
    ...
    plt.plot(x[:-2], (naive_BS_calls - naive_Bonly_calls)/naive_swaps, 'bo', label = "Naive")
    plt.plot(x[:-2], (naive_BS_FP1_calls - naive_Bonly_calls)/naive_swaps, 'go', label = "Naive + FP1")
    plt.plot(x, (UCB_BS_calls - UCB_Bonly_calls)/UCB_swaps , 'ro', label = "UCB")
    plt.plot(x, (UCB_BS_FP1_calls - UCB_Bonly_calls)/UCB_swaps , 'yo', label = "UCB + FP1")
    plt.plot(xticks_short, quadratic, 'b-')
    plt.plot(xticks_short, quadratic_FP1, 'g-')
    plt.plot(xticks, nlogn, 'r-')
    plt.plot(xticks, nlogn_FP1, 'y-')
    plt.legend()
    plt.title("Average number of distance calls per swap step vs. dataset size")
    plt.xlabel("N")
    plt.ylabel("Average number of d calls per swap step")
    plt.plot()
    plt.show()


# BUILD AND SWAP, SCALING WITH K: k: 2, 3, 4 and N = 3000
def build_and_swap_k():
    k = [2, 3, 4]
    UCB_swaps = np.array([1, 3, 2, 2, 5])
    naive_swaps = np.array([1, 3, 2])

    naive_BS_calls_FP1 = np.array([27058388, 45136507, 63244050])
    naive_BS_calls = np.array([36058388, 81163507, 144244050])
    ucb_BS_calls_FP1 = np.array([6889761, 15565246, 32008308])
    ucb_BS_calls = np.array([7223261, 20099646, 46805908])
    ...
    naive_Bonly_calls = naive_Bonly_calls_FP1
    ucb_Bonly_calls = ucb_Bonly_calls_FP1
    # Subtract p=True, N = 3000, Build only, k = 2, 3, 4

    naive_swaps_FP1 = np.array([1, 2, 3])

    linear_naive = 9e6*np.array(k)
    linear_naive_FP1 = 9e6*np.ones(3)
    linear_ucb = 0.5e7*(np.array(k) - 2) + 2e6
    linear_ucb_FP1 = 0.15e7*(np.array(k) - 1.5) + 2e6

    plt.plot(k, (naive_BS_calls - naive_Bonly_calls)/naive_swaps, 'bo', label = "Naive")
    plt.plot(k, (ucb_BS_calls - ucb_Bonly_calls)/naive_swaps, 'ro', label = "UCB")
    plt.plot(k, (naive_BS_calls_FP1 - naive_Bonly_calls_FP1)/naive_swaps_FP1, 'go', label = "Naive + FP1")
    plt.plot(k, (ucb_BS_calls_FP1 - ucb_Bonly_calls_FP1)/naive_swaps_FP1, 'yo', label = "UCB + FP1")

    plt.legend()
    plt.xlabel("k")
    plt.ylabel("Average number of d calls per swap step")
    plt.title("Average number of d calls per swap step vs. number of medoids")
    plt.plot(k, linear_naive, 'b-')
    plt.plot(k, linear_naive_FP1, 'g-')
    plt.plot(k, linear_ucb, 'r-')
    plt.plot(k, linear_ucb_FP1, 'y-')
    plt.show()
